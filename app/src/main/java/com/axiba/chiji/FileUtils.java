...
//                                    Log.d("TAG", "onScanCompleted: ");
    ...
    public static void savaFileToSD(Context context, Object base64Data) {
        try {
            String base64DataStr = (String) base64Data;
            int extStart = base64DataStr.indexOf("/") + 1;
            int extEnd = base64DataStr.indexOf(";");
            String ext = base64DataStr.substring(extStart, extEnd);
            if(TextUtils.isEmpty(ext)){
                ext = "jpg";
            }
            int dataStart = base64DataStr.indexOf("base64,") + "base64,".length();
            String data = base64DataStr.substring(dataStart);
            savaFileToSD(context, System.currentTimeMillis() + "." + ext, Base64.decode(data, Base64.NO_WRAP));
        } catch (Exception e) {
            Toast.makeText(context, "保存失败，图片各式错误！", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public static void savaFileToSD(Context context, String filename, byte[] bytes) {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            try {
                ...
                output.write(bytes);
                output.close();
                Toast.makeText(context, "图片已成功保存到相册" + filename1, Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(context, "保存出错！"+e.getMessage(), Toast.LENGTH_SHORT).show();
            }
// 最后通知图库更新
